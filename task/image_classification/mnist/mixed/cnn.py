...
def get_basenet_test_config() -> List[Dict[str, float]]:
    ...
    test_params_dict: Dict[str, List[float]] = {
        ...
        # Convolutional part
        'cnn_stage_num': [float(4)] * 2,
        'cnn_layer_num_per_stage': [float(2)] * 2,
        'stage_wise_pooling_type': [float(StageWisePoolingType.CONV_POOLING.value)] * 2,
        'conv_channel_dim': [float(16),
                             float(32)],
        'kernel_size': [float(3)] * 2,
        'group_num': [float(4)] * 2,
        'final_pooling_type': [float(InputPoolingReshapeType.AVG_POOLING.value)] * 2,
        # MLP part
        'hidden_num': [float(0)] * 2,
        'linear_size': [float(0)] * 2,
        # Common
